...
class PlotWidget(QWidget,ObserverWidget,Debugger):
    ...
    def removeFromCanvas(self,plot):
        ...
        if not self._threeD:
            ...
            if plot.style in ['Image (reg)']:               # test if it is an image
...
class Plot3DWidget(PlotWidget):
    ...
    def updatePlotData(self,draw=False,plot=None,regularize=None,**kwargs):
        ...
        if plot.MPLplot==None:                                                   # This is where any new 3D plot is created (as an empty plot)
            self.debugPrint('create empty plot ',plot)
            if plot.style=="Waterfall":
                plot.MPLplot= ax.plot([0],[0],0,'x')
            elif plot.style=="Image (reg)":
                plot.data={'x':array([0.]),'y':array([0.]),'z':array([[0.]]),'mask':True}             # note that z is always of type array and not masked array
                ...
                plot.data={'x':array([0.]),'y':array([0.]),'z':array([[0.]]),'mask':True}             # note that z is always of type array
            ...
            self.debugPrint('update existing plot ',plot)
            if plot.style=="Waterfall":
    ...
    def data2Plot(self,plot,regularize=None):
        ...
        if all([dim==1 for dim in dims]):                                                   # if x,y and z are 1d => no children => reshape the data with x or y in 2d
            le=min([len(vec) for vec in rowxyz])                                             #   first truncate x,y,or z at their common maximum length
            rowxyz=[vec[:le] for vec in rowxyz]
