...
class Plot3DWidget(QWidget,ObserverWidget):
  ...
  def updateNames(self,names):
    ...
    #for name in sorted(names):            # changed by DV in April 2014 to conserve the order
    for name in names:
  ...
  def __init__(self,parent = None):
    ...
    self.autoplot=QPushButton("Autoplot")
    ...
    self.connect(self.autoplot,SIGNAL("clicked()"),self.addDefaultPlot)
...
class Plot2DWidget(QWidget,ObserverWidget):
  ...
  def addPlot(self,xName=None,yName=None,**kwargs):
    ...
    if self._cube == None:
      return
    if self.autoclear.isChecked() : self.clearPlots() 
    self._plots.append(plot)
  ...
  def setDatacube(self,cube):
    ...
    self.selectDefaultNames()             # added DV April 2014 to select the X and Y default names
    if self.autoplot.isChecked(): self.addPlot()    
  ...
  def selectDefaultNames(self):             # added DV April 2014
    """"
    Selects automatically the first pair of X and Y default names if they exist in the active datacube.
    """
    if 'defaultPlot' in self._cube.parameters():
