...
#  version = "0.2"
  ...
  def defineDefaultPlot(self,x,y):
    """
    Define the default x and y columns.
    """
  ...
  def set(self,**keys):
    """
    Sets a set of variables in the current row
    """
  ...
  def setInOrder(self,**keys):
    """
    Sets a set of variables in the current row. Creates new columns if needed.
    Allows to force the order of column creation using a special keyword 'order' with value equal to a list of column names. Example. Starting froman empty datacube setInOrder(d=0,c=1,b=2,a=3,order=['b','a']) creates the columns 'd','c','a','b'
    """ 
    currentShape = shape(self._table)
    if self._meta["index"] >= currentShape[0]:
      self._resize((self._meta["index"]+10001,len(self._meta["fieldNames"])))
