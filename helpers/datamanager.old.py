...
class DataManager(Singleton,Reloadable,ThreadedDispatcher,Subject,Observer):
  ...
  """
  The DataManager is a Singleton class which can be used to keep track of datacubes. Just call addDatacube() to add a datacube to the DataManager.
  ...
  def plot(self,datacube,xname,yname,clear=False):
    """
    Trigger a plot of yname vs xname of the datacube
    """
  ...
  def autoPlot(self,datacube,clear = False):
    """
    Triggers an autoPlot of the selected datacube, i.e. a graphical frontend that observes the state of the data manager will be notified that 'datacube' should be replotted.
    """
